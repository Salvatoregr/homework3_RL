...
KDL::JntArray KDLRobot::getInvDifKin(KDL::JntArray &dq, const KDL::Twist &ee_vel)
{
    Eigen::Matrix<double,3,1> dot_p(ee_vel.vel.data);
    Eigen::Matrix<double,3,1> dot_r(ee_vel.rot.data);
...
{
   /* KDL::JntArray jntArray_out_;
    KDL::JntArray jntVel_out_;
    KDL::JntArray jntAcc_out_;
    int err_pos = ikSol_->CartToJnt(q, f, jntArray_out_);
    q = jntArray_out_;
    if (err_pos != 0)
    {
        printf("inverse kinematics failed with error: %d \n", err_pos);
    }
    int err_vel = ikVelSol_->CartToJnt(q, twist, jntVel_out_);
    dq = jntVel_out_;
    if (err_vel != 0)
    {
        printf("inverse kinematics failed with error: %d \n", err_vel);
